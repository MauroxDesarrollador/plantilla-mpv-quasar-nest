...
import {ICreateOne, IDeleteOne,IGetOne,IGetPaginate,IUpdateOne,IGetAll, GetOneResponse} from './prisma.service';
@Injectable()
...
/*   constructor(private readonly cache: CacheUtils) {
  ...
  async getOne(params:IGetOne):Promise<GetOneResponse|null>{
    try{
      const tiempoInicio = performance.now();
  ...
  async deleteOne(Params:IDeleteOne):Promise<object>{
    try{
      ...
      let data;
      if(searchData.hasOwnProperty("deletedAt")){
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
